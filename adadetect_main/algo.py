...
def BH(pvalues, level): 
    ...
    #get largest location k for which p(k) <= level * k / n
    #equivalent to getting smallest i0 such that p(k) >= level * k / n for all k > i0
    comp = comp[::-1] 
...
def adaptiveEmpBH(null_statistics, test_statistics, level, 
                correction_type='storey', storey_threshold=0.5,
    ...
    if return_threshold:
        R=BH(pvalues=pvalues, level= lvl_corr)
        n_rej=len(R)
        threshold= np.sort(test_statistics)[::-1][n_rej] if n_rej else 1 #assume statistics are in [0,1] here 
        return R, threshold #with respect to test statistics (and relative to null statistics values)
 
    else: return BH(pvalues=pvalues, level= lvl_corr)
