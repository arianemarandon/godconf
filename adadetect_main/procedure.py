...
class AdaDetectBase(object):
    ...
    def topk_posthoc_bound(self, x, level, xnull):
        self.fit(x, level, xnull)
        ...
        bounds=[np.minimum(1, 
                    np.min(k-np.arange(1,k+1) + m*pvalues[:k]/level)/k
...
class AdaDetectERM(AdaDetectBase):
    ...
    def fit(self, x, level, xnull):
        ...
        if isinstance(x, list):
            x_mix_train=x+ xnull_calib
            x_train=xnull_train+ x_mix_train
...
class AdaDetectERMcv(AdaDetectBase): 
    ...
    """
    def __init__(self, scoring_fn, cv_params=None, split_size=0.5, split_size_cv=0.5):
        """
        ...
        self.split_size = split_size
        self.split_size_cv = split_size_cv
    ...
    def fit(self, x, level, xnull):
        ...
        if self.cv_params is not None:
            ...
            #n_calib_2 = n-n_null_train #l
            n_null_train_1 = int(self.split_size_cv * n_null_train ) # s
            n_calib_1 = n_null_train-n_null_train_1
            #s = n_null_train - n_calib_1
            ...
            xnull_calib_2 = xnull[n_null_train:] #Y_(k+1), ... Y_n 

            new_x = np.concatenate([x, xnull_calib_2])#Z_(k+1), ..., Z_(n+m)

            new_x_null = np.concatenate([xnull_train, xnull_calib_1]) #Z_1,...,Z_s, ..., Z_k
            grid = list(ParameterGrid(self.cv_params))
            max_power=0
            best_params=None

            split_size = len(xnull_train) / len(new_x_null) #s/(k+s)
            ...
            xnull_train = new_x_null #Y_1, ..., Y_k = Z_1, ..., Z_k
            x_mix_train = new_x    #Z_(k+1), ... Z_(n+m) 
